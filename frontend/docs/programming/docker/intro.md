...
::: info 들어가며
꽤 오랫동안 개발자로 일했지만 저는 Docker에 대해서 아는 것이 별로 없었습니다. 그저 배포에 필요한 도구 중 하나라고 짐작만 할 뿐이었죠. 요즘 채용정보를 보면 자격요건에 심심치 않게 Git과 Docker가 등장하는 것을 볼 수 있습니다. 이 포스팅에서는 Docker에 대해 간략하게 알아보는 시간을 갖도록 해봅시다.
:::
...
회사나 개인 프로젝트를 진행하다 보면 실제 서비스를 제공하기 위해 배포할 일이 생깁니다. 개발자로 일하면서 누구나 한 번쯤은 배포를 해본 경험이 있을 텐데요. 제 기억에 배포는 정말 귀찮은 일이었습니다.
* 빌드하고
* 압축하고
* 서버에 붙어 파일을 옮기고
* 기존 서비스를 중지하고
