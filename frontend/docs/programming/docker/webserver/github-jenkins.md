...
Jenkins와 연동에 사용할 **jenkins**라는 Node.js 프로젝트를 하나 만들겠습니다.
```shell
$ npm init
```
...
|:--:|
| *Jenkins App 실행*{:class='caption'} |

## Github 저장소 만들기
이제 Github에 프로젝트 저장소를 만들고 소스를 올려보도록 하겠습니다.

Github Dashboard 좌측에 `Create repository`를 클릭하여 저장소를 생성해 줍니다.

|![Github 저장소 만들기](./images/github-jenkins/github01.webp){:class='image'}|
|:--:|
| *Github 저장소 만들기*{:class='caption'} |

만들어진 저장소로 코드를 올려보겠습니다.
```shell
$ git init
$ git add .
...
|:--:|
| *Github 저장소에 소스 올리기*{:class='caption'} |
...
키가 생성될 위치와 키 이름을 지정하고 그 다음 부분은 엔터키를 눌러 Skip 해줍니다.

|![SSH 인증키 쌍 생성](./images/github-jenkins/github03.webp){:class='image'}|
|:--:|
| *SSH 인증키 쌍 생성*{:class='caption'} |

`Jenkins`라는 이름으로 개인키와 공개키 한 쌍이 생성되었습니다.

## Github Deploy Key 등록
생성된 공개키를 Github의 Deploy Keys에 등록해 보도록 하겠습니다.

Github Jenkins 저장소 우측 상단의 `Settings`를 선택해 `Deploy Keys`로 이동합니다.
...
|:--:|
| *Github Deploy Key 등록 완료*{:class='caption'} |

## Jenkins Credential 등록
생성된 개인키를 Jenkins Credential에 등록해 보도록 하겠습니다.

**Jenkins Dashboard** > **Jenkins 관리** > **Credentials** 로 차례대로 이동합니다.

System credential 행 Domains 컬럼의 **(global)** 부분에 마우스를 오버해서 드롭 다운하여 `Add credentials`를 선택합니다.
...
|:--:|
| *Jenkins Credential 등록완료*{:class='caption'} |

## Jenkins 프로젝트 추가 및 Github 연동
이제 Jenkins 프로젝트를 추가해 Github 주소를 연결하고 위에서 만들어진 Github Deploy Key와 Jenkins Credential을 연동해 보도록 하겠습니다.

Jekins Dashboard 화면에서 `+ 새로운 Item`을 선택합니다.
|![Jenkins 프로젝트 추가](./images/github-jenkins/connection01.webp){:class='image'}|
|:--:|
| *Jenkins 프로젝트 추가*{:class='caption'} |

**Enter an item name** 항목에 Jenkins에 추가할 프로젝트 명을 입력하고, **Freestyle project**를 선택해 `OK` 버튼을 클릭합니다.
...
> Branched to build 항목에 지정한 브랜치가 push 됐을 경우 작동합니다.
...
> [!INFO] Build Steps
> Build Steps > **Execute shell**을 이용해 Push 이후 Docker 이미지를 생성하고 이미지를 컨테이너로 구동하는 등의 후처리 작업을 정의할 수 있습니다.
...
|:--:|
| *Jenkins 프로젝트 추가 완료*{:class='caption'} |

Jenkins 프로젝트 추가 및 Github 연동 작업이 완료되었습니다.

## Github Webhook 등록
Github Jenkins 저장소 우측 상단의 `Settings`를 선택해 `Webhooks`로 이동합니다.
...
|:--:|
| *Github Webhook 등록 완료*{:class='caption'} |

## Node.js 앱 배포 테스트
모든 연동 작업이 완료되었습니다.

Node.js 앱을 push 해서 정상적으로 Jenkins와 연동되는지 확인해 봅시다.
```shell
$ git push origin main
```
<br />

Jenkins Dashboard 페이지 빌드 대기 목록에 추가한 프로젝트가 활성화됩니다.
|![Node.js 앱 배포 테스트](./images/github-jenkins/test01.webp){:class='image'}|
|:--:|
| *Node.js 앱 배포 테스트*{:class='caption'} |
